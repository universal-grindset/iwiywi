...
// `mode`, `ok`, and `err` round out the palette for future use (status lines,
// success/error toasts). Kept as load-bearing struct fields so the API of
// `Theme` is stable even before all colors are consumed.
#[expect(
    dead_code,
    reason = "palette completeness; future renderers will use mode/ok/err"
)]
pub struct Theme {
    pub mode: Mode,
    pub accent: Color,
